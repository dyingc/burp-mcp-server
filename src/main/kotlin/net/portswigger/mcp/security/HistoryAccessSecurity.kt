...
enum class HistoryAccessType() {
    HTTP_HISTORY(), WEBSOCKET_HISTORY(), LOGGER_HTTP_HISTORY();
}
        ...
        return suspendCoroutine { continuation ->
            SwingUtilities.invokeLater {
                val historyTypeName = when (accessType) {
                    ...
                    HistoryAccessType.LOGGER_HTTP_HISTORY -> "Logger HTTP history"
                }
                ...
                when (result) {
                    ...
                    1 -> {
                        when (accessType) {
                            ...
                            HistoryAccessType.LOGGER_HTTP_HISTORY -> config.alwaysAllowLoggerHistory = true
                        }
                        continuation.resume(true)
                    }
...
object HistoryAccessSecurity {
    ...
    suspend fun checkHistoryAccessPermission(
        ...
        val isAlwaysAllowed = when (accessType) {
            ...
            HistoryAccessType.LOGGER_HTTP_HISTORY -> config.alwaysAllowLoggerHistory
        }
